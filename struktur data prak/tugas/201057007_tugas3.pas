...
var	bayangan,buku:arr_buku;
    bayanganp,penjualan:arr_penjualan;
    pil_menu,n,np,i,j:Integer;
...
begin
    ...
    {cetak hasil sorting};
    writeln('----DATA BUKU SESUDAH DIURUTKAN BERDASAR JUDUL ---:');writeln;
    tampilkanBuku(bayangan);
end;


procedure urut_data_penjualan(var X:arr_penjualan); {sorting}
var dummy: rec_penjualan;
begin
    {kirim buku ke record bayangan}
    for i:=1 to np do bayanganp[i]:=x[i];
    {proses sorting}
    for i:=1 to np-1 do
    begin
        for j:=i+1 to np do
        begin
            if bayanganp[i].judul_buku>bayanganp[j].judul_buku then
                begin
                    dummy:=bayanganp[i];
                    bayanganp[i]:=bayanganp[j];
                    bayanganp[j]:=dummy;
                end;
        end;
    end;
...
begin
    ...
    repeat
      ...
      WriteLn('8.urutkan data buku berdasarkan judul'); 
      WriteLn('9.urutkan data penjualan berdasaarkan judul'); 
      WriteLn('0.Keluar');
      ...
      7:if np=0 then WriteLn('database penjualan kosong') else tampilkanPenjualan(penjualan);
      8:if n=0 then WriteLn('database kosong') else urut_data(buku);
      9:if np=0 then WriteLn('database penjualan kosong') else urut_data_penjualan(penjualan);
