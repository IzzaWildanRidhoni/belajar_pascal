...
// procedure urut nomer
procedure urut_nomorpst(var X:larik_toefl);
var dummy:toefl;
begin
    //   mengisi data di larik  record Y
    for i:=1 to n do Y[i]:=X[i];
    // kita akan mengurutkan Y instead X
    WriteLn('cetak data sebelum diurutkan');
    cetak_pst(Y);
    WriteLn;
    for  i:=1 to n-1 do 
    begin
      for j:=i+1 to n do  
      begin
        if Y[i].no_pst > Y[j].no_pst then
        begin
            dummy:=Y[i];Y[i]:=Y[j];Y[j]:=dummy;
        end;
      end;
    end;
...
// procedure urut skor toefl
procedure urut_skor(var X:larik_toefl);
var dummy:toefl;sudah:Boolean;
begin
    //   mengisi data di larik  record Y
    for i:=1 to n do Y[i]:=X[i];

    // cek apakah peserta blm ada yang melakukan tes
    sudah:=False;
    for i:=1 to n do 
    begin
      if Y[i].total<>0 then sudah:=true;
    end;
    if sudah then
    begin
        // kita akan mengurutkan Y instead X
    ...
    else WriteLn('belum ada peserta yang melakukan tes toefl');
end;
...
repeat
  ...
  writeln('10. urutkan data berdasarkan no peserta ');//mengurutkan methode straight exchange
  ...
  case pil of
    ...
    10: if n=0 then WriteLn('Peserta belum ada ') else urut_nomorpst(peserta);
    11: if n=0 then WriteLn('Peserta belum ada ') else urut_skor(peserta);
